(function () {
  ...
  let quickSendList = [
    {
      ...
      list: [
        ...
        {
          name: "Query UART Config",
          content: "AT+UART_CUR?",
          hex: false,
        },
        {
          name: "Set UART 115200 Save flash",
        ...
        {
          name: "Query System Help",
          content: "AT+SYSMSG?",
          hex: false,
        },
        {
          name: "Query System Flash",
          content: "AT+SYSFLASH?",
          hex: false,
        },
        {
          name: "Query System Timestamp",
  ...
  document.getElementById("serial-auto-scroll").innerText = toolOptions.autoScroll ? "Auto Scroll" : "Stop Scroll";
  ...
  document.getElementById("serial-auto-scroll").addEventListener("click", function (e) {
    let autoScroll = this.innerText != "Auto Scroll";
    this.innerText = autoScroll ? "Auto Scroll" : "Stop Scroll";
  ...
  document.getElementById("serial-select-port").addEventListener("click", async () => {
    ...
    try {
      ...
      console.error("Get Serial Fail" + e.toString());
    }
  });
  ...
  async function closeSerial() {
    if (serialOpen) {
      serialOpen = false;
      reader?.cancel();
      serialToggle.innerHTML = "Open Serial";
  ...
  async function openSerial() {
    ...
    serialPort
      .open(SerialOptions)
      .then(() => {
        serialToggle.innerHTML = "Stop Serial";
        serialOpen = true;
        serialClose = false;
      ...
      .catch((e) => {
        showMsg("Open Serial Fail:" + e.toString());
      });
  }
  ...
  function serialStatuChange(statu) {
    let tip;
    if (statu) {
      tip = '<div class="alert alert-success" role="alert">Device Connected</div>';
    } else {
      tip = '<div class="alert alert-danger" role="alert">Device Disconnected</div>';
    }
  ...
  async function send() {
    ...
    if (!content) {
      addLogErr("Send message is null ");
      return;
    }
  ...
  async function writeData(data) {
    if (!serialPort || !serialPort.writable) {
      addLogErr("Please first open serial, then send msg.");
      return;
    }
  ...
  function addLogErr(msg) {
    let time = toolOptions.showTime ? formatDate(new Date()) + "&nbsp;" : "";
    const template = '<div><span class="text-danger">' + time + " System Msg</span><br>" + msg + "</div>";
    let tempNode = document.createElement("div");
    tempNode.innerHTML = template;
    serialLogs.append(tempNode);
    if (toolOptions.autoScroll) {
