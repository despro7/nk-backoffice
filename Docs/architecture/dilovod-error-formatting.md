...
Помилка блокування паралельних запитів до Dilovod API. На рівні `DilovodApiClient` запити серіалізуються **глобальною чергою** (одна на процес) — див. `server/services/dilovod/README.md`. Якщо penalty все ж отримано, черга ставить `globalPauseUntil` на 30 с і retry з backoff.

У UI обробляється як硬помилка (не форматується, просто показується стандартне повідомлення).
