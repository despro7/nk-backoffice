...
// Use loose `any` types because write-off history shape differs from returns types

// ---------------------------------------------------------------------------
// WriteOffHistoryTable — список списань як акордеон (копія ReturnsHistoryTable для модуля списань)
// ---------------------------------------------------------------------------

interface WriteOffHistoryTableProps {
  records: any[];
  onLoadRecord?: (record: any) => Promise<void>;
  onDeleteRecord?: (recordId: string) => Promise<void>;
}

export const WriteOffHistoryTable = ({ records, onLoadRecord, onDeleteRecord }: WriteOffHistoryTableProps) => {
  const { user } = useAuth();
  ...
  if (records.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400">
        ...
        <p>Немає завершених списань</p>
      </div>
    );
  }

  const getReasonLabel = (reason: string, customReason?: string) => {
    if (!reason && customReason) return customReason;
  ...
  const handleLoadRecord = async (recordId: string) => {
    ...
    try {
      ...
      console.error('[WriteOffHistoryTable] Error loading record:', error);
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error('[WriteOffHistoryTable] Error deleting record:', error);
    } finally {
            ...
            >
              <div className="flex items-center gap-2 flex-1">
                ...
                <Chip size="sm" color="default" variant="flat">
                  {getReasonLabel(record.writeOffReason, record.customReason)}
                </Chip>
              </div>

              <div className="flex items-center gap-4">
                  <div className="grid grid-cols-2 w-72 gap-8 ml-2 text-xs text-gray-500">
              ...
              >
                        ...
                        {onDeleteRecord && (
                          <>
                            ...
                            <ConfirmModal
                              ...
                              title="Видалити запис списання?"
                              message={`Запис списання №${record.id} буде видалений безповоротно.`}
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-2 bg-gray-50 justify-between px-3 py-2 rounded-t-md border-1 border-b-0 border-gray-200">
                    <h4 className="text-md font-medium text-gray-700">Товари для списання</h4>
                  </div>
                      ...
                      <tbody>
                        {items.map((item, idx) => {
                          ...
                          <tr key={idx} className="border-b not-last:border-b-gray-100 hover:bg-white text-gray-700 transition-colors">
                            ...
                            <td className="py-2 px-3">{item.batchNumber || item.batchId || '–'}</td>
                        ...
                        <tr className="bg-gray-100/60">
                          ...
                          <td className="text-center font-semibold py-2">{items.reduce((s, it) => s + Number(it.quantity ?? it.qty ?? 0), 0)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
