...
## Монолітні набори vs порції

Джерело істини для форми повернення — `orders.payloadData.shipment.bySku` (як при відвантаженні).

| Умова | Поведінка повернення |
|---|---|
| SKU набору є в `payloadData.shipment.bySku` | Повертається цілим набором (`shippedAsMonolithic`) |
| Запису немає (відвантажено порціями) | Набір розгортається на окремі компоненти/порції |

Реалізація (`useWarehouseReturns` → `expandProductSets`):

- Завжди `useShipmentPayloadMode: true`, навіть якщо `bySku` відсутній або порожній.
