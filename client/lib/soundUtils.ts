...
export type { CueSoundName } from './soundRecipes';
export { CUE_SOUNDS, isCueSoundName } from './soundRecipes';
...
const CUE_OUTPUT_GAIN = 4;

let cueOutput: GainNode | null = null;
...
function getCueOutput(context: AudioContext): GainNode {
  if (cueOutput) return cueOutput;

  const output = context.createGain();
  output.gain.value = CUE_OUTPUT_GAIN;
...
function renderRecipe(context: AudioContext, recipe: SoundRecipe, volume: number): void {
  ...
  const output = getCueOutput(context);
...
/** Синтез рецепта Cuelume на спільному AudioContext застосунку. */
export function playCueSound(name: CueSoundName, volume = 1): void {
  ...
  ensureAudioContextResumed().then((audioContext) => {
    ...
    try {
      renderRecipe(audioContext, RECIPES[name], playVolume);
    } catch (error) {
      console.warn('Не вдалося відтворити cue-звук:', error);
    }
  });
}
...
export const SOUND_CHOICES = [
  ...
  { value: 'chime_legacy', label: 'Дзвоник (старий)' },
  ...
  { value: 'chime', label: 'Cuelume: Дзвоник' },
  { value: 'sparkle', label: 'Cuelume: Іскорка' },
  { value: 'droplet', label: 'Cuelume: Крапля' },
  { value: 'bloom', label: 'Cuelume: Пад' },
  { value: 'whisper', label: 'Cuelume: Шепіт' },
  { value: 'tick', label: 'Cuelume: Тік' },
  { value: 'press', label: 'Cuelume: Натиск' },
  { value: 'release', label: 'Cuelume: Відпускання' },
  { value: 'toggle', label: 'Cuelume: Перемикач' },
  { value: 'success', label: 'Cuelume: Успіх' },
  { value: 'error', label: 'Cuelume: Помилка' },
  { value: 'page', label: 'Cuelume: Сторінка' },
  { value: 'loading', label: 'Cuelume: Завантаження' },
  { value: 'ready', label: 'Cuelume: Готово' },
  { value: 'pulse', label: 'Cuelume: Пульс' },
  { value: 'scan', label: 'Cuelume: Сканування' },
  { value: 'arrival', label: 'Cuelume: Прибуття' },
];

export const CUE_SOUND_CHOICES = CUE_SOUNDS.map((value) => ({
  value,
  label: SOUND_CHOICES.find((item) => item.value === value)?.label ?? value,
}));
